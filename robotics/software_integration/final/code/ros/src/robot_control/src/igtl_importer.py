...
class importer():
    ...
    def cb_point(self, data):
        ...
        if data.name == 'Entry':
            ...
            rospy.loginfo(rospy.get_caller_id() + " Entry = (%f, %f, %f).", data.pointdata.x, data.pointdata.y,
                          data.pointdata.z)
            ...
            rospy.loginfo(rospy.get_caller_id() + " Target = (%f, %f, %f).", data.pointdata.x, data.pointdata.y,
                          data.pointdata.z)
    ...
    def execute(self):
        vectorET = [self.target.position.x - self.entry.position.x, self.target.position.y - self.entry.position.y,
                    self.target.position.z - self.entry.position.z]
        vectorET = vectorET / self.magnitudeVector(vectorET)
        vectorO = [0.0, 1.0, 0.0]
        vectorX = np.cross(vectorO, vectorET)
        vectorY = np.cross(vectorET, vectorX)
        orientationMatrix = np.mat([[vectorX[0], vectorY[0], vectorET[0]], [vectorX[1], vectorY[1], vectorET[1]],
                                    [vectorX[2], vectorY[2], vectorET[2]]])
