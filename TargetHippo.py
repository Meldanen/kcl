...
class TargetHippo(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.title = "TargetHippo" # TODO make this more human readable by adding spaces
...
class TargetHippoWidget(ScriptedLoadableModuleWidget):
...
class TargetHippoLogic(ScriptedLoadableModuleLogic):
  ...
  def run(self, inputVolume, outputVolume):
    ...
    hippoArray = slicer.util.arrayFromVolume(hippo)

    [xmax,ymax,zmax] = hippo.GetImageData().GetDimensions()

    goodTarget = []
    
    for x in range(0,targetsNode.GetNumberOfMarkups()):
      world = [0,0,0,0]
      targetsNode.GetNthFiducialWorldCoordinates(x,world)
      subsampleFactor = 4
      ...
      imageVal = hippo.GetImageData().GetScalarComponentAsDouble(xIndex,yIndex,zIndex,0)
      print(imageVal)
...
class TargetHippoTest(ScriptedLoadableModuleTest):
  """
  ...
  def runTest(self):
    ...
    """
    self.setUp()
    self.test_TargetHippo1()

  def test_TargetHippo1(self):
